...
const GIT_BACKUP = path.join(os.tmpdir(), 'booksok_git_backup');
const version = Date.now().toString();

console.log(`\n빌드 시작 (버전: ${version})\n`);

// 1. .git 임시 보존 (expo export가 dist/ 초기화하므로)
...
if (!fs.existsSync(GIT_DIR)) {
  ...
  execSync('git remote add origin git@github.com:keys0919/chchaeybook.git', { cwd: DIST });
